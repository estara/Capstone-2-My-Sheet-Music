...
describe("GET /userLib/:id", function () {
  test("works for admin", async function () {
    const resp = await request(app)
      .get(`/userLib/${testWorksOwnedIds[0]}`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
      work: {
        id: testWorksOwnedIds[0],
        owned: false,
        digital: false,
        physical: true,
        played: false,
  ...
  test("works for correct user", async function () {
    const resp = await request(app)
      .get(`/userLib/${testWorksOwnedIds[1]}`)
      .set("authorization", `Bearer ${u2Token}`);
    expect(resp.body).toEqual({
      work: {
        id: testWorksOwnedIds[1],
        owned: true,
        digital: true,
        physical: true,
        played: true,
  ...
  test("unath for anon", async function () {
    const resp = await request(app).get(`/userLib/${testUserIds[1]}`);
    expect(resp.statusCode).toEqual(401);
  });
...
describe("PATCH /userLib/:id", function () {
  ...
  test("works for correct user", async function () {
    const resp = await request(app)
      .patch(`/userLib/${testWorksOwnedIds[1]}`)
      .send({
        loanedout: true,
      })
      .set("authorization", `Bearer ${u2Token}`);
...
describe("DELETE /userLib/:id", function () {
  ...
  test("unauth for anon", async function () {
    const resp = await request(app).delete(`/userLib/${testWorksOwnedIds[0]}`);
    expect(resp.statusCode).toEqual(401);
  });
